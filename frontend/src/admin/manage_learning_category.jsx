...
import "../css/admin/sort.css";
import { useNavigate } from "react-router-dom";
import { ToastContainer, toast } from "react-toastify";
import "react-toastify/dist/ReactToastify.css";
import { DragDropContext, Droppable, Draggable } from "react-beautiful-dnd";

export default function ManageLearningCategory() {
  ...
  const handleShowSort = () => {
    fetchCategories(); // Fetch data when modal opens
    setShowSort(true);
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await fetch(
        `${APP_URL}/api/categories/${selectedCategoryId}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${yourToken}`,
      ...
      toast.success(
        "Category and associated Topic and Learning Materials deleted successfully."
      );
      handleClose_delete();
    } catch (error) {
  ...
  const onSubmitEdit = async (data) => {
    console.log(data); // Log the form data for editing
    ...
    try {
      const response = await fetch(
        `${APP_URL}/api/categories/${edit_selectedCategoryId}`, // Use edit_selectedCategoryId
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  //   try {
  //     const updatedCategories = categoryList.map((category, index) => ({
  //       id: category.id,
  //       sort_id: index + 1,
  //     }));

  //     const response = await fetch(
  //       "http://localhost:8000/api/categories/update-sort-order",
  //       {
  //         method: "POST",
  //         headers: {
  //           "Content-Type": "application/json",
  ...
  return (
    <>
      ...
      <section className="basic-info-section">
        <Title title="Manage Learning Categories" />
        ...
        <div className="other-action-container">
          ...
          <div className="btn-container">
            ...
            {/* <button onClick={handleShowSort}>Sort Category</button> */}
          </div>
        </div>
          ...
          <table>
            <thead>
              <tr>
                ...
                {/* <th className="col-lg-2 col-xl-2">Sort ID</th> */}
      ...
      {/* Modal for sorting category */}
      {/* <Modal show={showSort} onHide={handleCloseSort} centered>
        <Modal.Header closeButton>
          <Modal.Title>Sort Categories</Modal.Title>
        </Modal.Header>
        <Modal.Body>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title className="modal-title-center">
            Add Category for Learning Materials
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
